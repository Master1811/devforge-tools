...
function inferType(value: any, key: string, interfaces: InterfaceMap, useExport: boolean, useType: boolean): string {
  ...
  if (typeof value === "object") {
    ...
    generateInterface(value, name, interfaces, useExport, useType);
    return name;
  }
  return typeof value;
}

function generateInterface(obj: Record<string, any>, name: string, interfaces: InterfaceMap, useExport: boolean, useType: boolean): void {
  const lines: string[] = [];
...
export function jsonToTypescript(json: string, options: { useExport?: boolean; useType?: boolean } = {}): string {
  ...
  if (Array.isArray(parsed)) {
    if (parsed.length > 0 && typeof parsed[0] === "object") {
      generateInterface(parsed[0], "Root", interfaces, useExport, useType);
    }
    return Object.values(interfaces).reverse().join("\n\n") + `\n\n${useExport ? "export " : ""}type RootArray = Root[];`;
  }
  generateInterface(parsed, "Root", interfaces, useExport, useType);
  return Object.values(interfaces).reverse().join("\n\n");
}
